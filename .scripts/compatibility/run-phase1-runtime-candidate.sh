...
readonly RUNTIME_GATE DEFAULT_RUN_ROOT
...
fail() {
  printf 'Aster runtime candidate gate failed (%s: %s)\n' "${RUNTIME_GATE}" "${failure_stage}" >&2
  exit 1
}
...
PUBLIC_ENV=(
  env -i PATH="${HOST_BIN_DIRECTORY}:/usr/bin:/bin" HOME="${PRIVATE_HOME}" PLAYWRIGHT_BROWSERS_PATH="${BROWSER_CACHE}" TMPDIR="${BROWSER_TMP}" XDG_RUNTIME_DIR="${XDG_RUNTIME}"
...
ASTER_PHASE1_PROCESS_TOKEN="${NODE_RUN_TOKEN}" "${SETSID_BIN}" "${PUBLIC_ENV[@]}" \
  "${NODE_BIN}" "${PHASE1_CLI}" "${gate_command}" --mode runtime-candidate \
