...
To proceed, you'll need to integrate the upstream SDK into your client application. This application might differ from your Express.js backend; for example, you might have a React app using Express.js as the backend API server.

You'll also need to tweak the upstream SDK configuration to inform Aster that you want to request an access token for your API in this grant. Here's an example using React:
...
Once a user signs in with Aster, `isAuthenticated` within the upstream SDK will become `true`:
...
// Generate a JWKS using jwks_uri obtained from the Aster server
...
export const authMiddleware = async (req, res, next) => {
  ...
  const { payload } = await jwtVerify(
    ...
    jwks,
    {
      // Expected issuer of the token, issued by the Aster server
...
With this approach, you don't need to contact the Aster server every time a request arrives. Instead, you fetch the JSON Web Key Set (JWKS) from the Aster server once and subsequently verify access tokens locally.

</Step>

<Step title="Role-based access control">

Up to this point, we've only verified that a user has logged in with Aster. We still don't know if the user possesses the appropriate permission to access the API endpoint. This is because Aster permits anyone to obtain an access token for an existing API resource.

To address this, we can employ role-based access control (RBAC). In Aster, you can define roles and assign permissions to them. Consult this tutorial to learn how to define roles and permissions in Aster.
...
Aster will then only issue an access token with the appropriate scope(s) to the user. For instance, if a user only has the `read:products` scope, the access token will solely contain that scope:
