...
<Step
  title="Installation"
  subtitle="Install the upstream SDK for your project"
>
...
sequenceDiagram
  ...
  participant C as Aster sign-in experience

  A->>B: Invokes sign-in
  B->>C: Redirects to Aster
  C->>C: User signs in
...
The upstream SDK requires the following permissions in the `manifest.json`:
...
Replace `https://auth.example.com` with your Aster endpoint.
...
- `host_permissions`: Required for the upstream SDK to communicate with the Aster APIs.

<InlineNotification>
If your Aster endpoint uses a custom domain, update the `host_permissions` to match that domain.
</InlineNotification>

### Set up a background script (service worker)

In your Chrome extension's background script, initialize the upstream SDK:

```js title="service-worker.js"
import LogtoClient from '@logto/chrome-extension';

export const logtoClient = new LogtoClient({
  endpoint: '<your-aster-endpoint>',
  appId: '<your-aster-app-id>',
});
```

Replace `<your-aster-endpoint>` and `<your-aster-app-id>` with the actual values. You can find these values in the application page you just created in the Aster Console.
...
<Step title="Update Aster application settings">

As we mentioned in the previous step, we need to update the Aster application settings to allow the redirect URIs we just created (`https://<extension-id>.chromiumapp.org/callback`):
...
Finally, the CORS allowed origins should include the extension's origin (`chrome-extension://<extension-id>`). The SDK in Chrome extension will use this origin to communicate with the Aster APIs.
...
3. You will be redirected to the Aster sign-in page.
4. Sign in with your Aster account.
...
Since Chrome provides unified storage APIs, all upstream SDK methods other than the sign-in and sign-out flows can be used directly in the popup page.
...
- **Module resolution**: Aster Chrome extension SDK is an ESM-only module.
