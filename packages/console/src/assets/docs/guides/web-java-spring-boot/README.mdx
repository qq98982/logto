...
This tutorial will show you how to integrate Aster into your Java Spring Boot application.

No official SDK is required to integrate Aster with your Java Spring Boot application. We will use the [Spring Security](https://spring.io/projects/spring-security) and [Spring Security OAuth2](https://spring.io/guides/tutorials/spring-boot-oauth2) libraries to handle the OIDC authentication flow with Aster.
...
<Step title="Configure Aster OAuth2 Client registration">
  
Register your application with Aster to get the client credentials and IdP configurations.
...
<Step title="Setup the redirect URI in Aster">
...
Make sure the redirect URI in Aster matches the `redirect-uri` set in the `application.properties` file in the previous step.
...
This is required because Aster uses ES384 as the default algorithm, we need to update the OidcIdTokenDecoderFactory to use the same algorithm.
...
<body>
  ...
  <p><a th:href="@{/oauth2/authorization/logto}">Login with Aster</a></p>
</body>
```
