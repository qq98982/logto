...
<InlineNotification>
  ...
  you DO NOT need to add additional libraries from Aster to integrate.
...
All the Aster Authorization server configurations can be found by requesting{' '}
...
- `spring.security.oauth2.resourceserver.jwt.issuer-uri`: The iss claim value and the issuer URI in the JWT issued by Aster.
...
public class SecurityConfiguration {
    ...
    public JwtDecoder jwtDecoder() {
        NimbusJwtDecoder jwtDecoder = NimbusJwtDecoder.withJwkSetUri(jwksUri)
                // Aster uses the ES384 algorithm to sign the JWTs by default.
                .jwsAlgorithm(ES384)
