...
This tutorial will show you how to integrate Aster into your PHP web application.
...
By default, the SDK uses the built-in PHP session to store the Aster data. If you want to use other storage, you can pass a custom storage object as the second parameter:
...
After the user signs in, Aster will redirect the user to the callback URL you set in the Aster Console. In this example, we use `/callback` as the callback URL:
...
Now, whenever your users visit `/sign-in`, it will start a new sign-in attempt and redirect the user to the Aster sign-in page.
...
To clean up the Python session and Aster session, we can designate a post sign-out redierct URI. This is where Aster will redirect users after they sign out.
...
The post sign-out redierct URI is optional, and if not provided, the user will be redirected to an Aster default page after a successful sign-out (without redirecting back to your application).

</Step>

<Step title="Handle authentication status">

In the upstream SDK, we can use `$client->isAuthenticated()` to check the authentication status, if the user is signed in, the value will be true, otherwise, the value will be false.
...
Route::get('/userinfo', function () {
  ...
  return (
    ...
    . "<br>"
    // Fetch user info from Aster userinfo endpoint
...
To learn more about scopes and claims, see Get user information.
