...
sequenceDiagram
    ...
    Device->>Aster: POST /oidc/device/auth
    Aster-->>Device: device_code, user_code, verification_uri

    Device->>User: Display user_code + verification_uri (or QR code)

    loop Poll for tokens
        Device->>Aster: POST /oidc/token (device_code)
        Aster-->>Device: authorization_pending
    end

    User->>Aster: Open verification_uri, enter user_code, sign in
    Aster-->>User: Authorization complete

    Device->>Aster: POST /oidc/token (device_code)
    Aster-->>Device: access_token, id_token, refresh_token
```
...
Replace `YOUR_API_RESOURCE_INDICATOR` with the API resource indicator (URI) you created in Aster.
