...
| Upstream SDK version | Dart SDK version  | Dart 3.0 compatible |
| ----------------- | ----------------- | ------------------- |
...
Behind the scenes, this SDK uses [flutter_web_auth_2](https://pub.dev/packages/flutter_web_auth_2) to authenticate users with Aster. This package provides a simple way to authenticate users with Aster using the system webview or browser.
...
- Android: Register the callback url on Android
  ...
  In order to capture the callback url from Aster's sign-in web page, you will need to register your sign-in redirectUri to your `AndroidManifest.xml` file.
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      title: 'Flutter Demo',
      home: MyHomePage(title: 'Aster Demo Home Page'),
    );
  }
}
...
- For Android, in order to capture the callback url from Aster's sign-in web page, you will need to register your sign-in redirectUri to the `AndroidManifest.xml`.
...
Just like signing in, users should be redirected to Aster for signing out of the shared session. Once finished, it would be great to redirect the user back to your website. For example, add  {`${props.defaultUri ?? defaultRedirectUri}`} as the post sign-out redirect URI below.
...
In your application, you can use the `logtoClient.signIn` method to initiate the sign-in process. This method will redirect the user to the Aster sign-in page and handle the authentication flow.

<Code className="language-dart" title="lib/main.dart">
{`class _MyHomePageState extends State<MyHomePage> {
  // ...
  final redirectUri = 'aster://callback';
...
{`class _MyHomePageState extends State<MyHomePage> {
  ...
  final postSignOutRedirectUri = 'aster://home';
...
In the upstream SDK, you can use `logtoClient.isAuthenticated` to check the authentication status, if the user is signed in, the value will be `true`, otherwise, the value will be `false`.
