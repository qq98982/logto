...
<Step
  title="Installation"
  subtitle="Install the upstream SDK"
>
...
When mounting the routes as described above, you can navigate your browser to `/api/logto/sign-in` and you should be redirected to your Aster instance where you have to authenticate then.
...
We have prepared the authentication routes, now let's implement the sign-in and sign-out buttons in your home page. We need to redirect the user to the sign-in or sign-out route when needed. To help with this, use `loader` to fetch authentication status from Aster client.
