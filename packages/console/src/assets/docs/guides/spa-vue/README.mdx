...
<InlineNotification>
  the upstream Vue SDK is built with Vue 3 composition API. Therefore, only Vue 3 is supported at the moment. Contact us if you want to add support for Vue 2.
</InlineNotification>

Import and use `createLogto` to install the upstream plugin:
...
There are still things to do after the user is redirected back to your application from Aster. Let's handle it properly.
...
Calling `.signOut()` will clear all the Aster data in memory and localStorage if they exist.
