...
If everything is configured correctly, once you complete the sign-in or registration process in Aster, you will be redirected back to ChatGPT. From now on, every request sent by ChatGPT to your plugin server will carry the Authorization header, allowing you to decode and verify the token in your API.
