...
<Step
  title="Install the upstream SDK"
>

Use your favorite package manager to install the upstream SDK:
...
In your `hooks.server.ts` file, add the following code to inject the Aster hook into your server:
...
Now you can access the Aster client in the `locals` object. For TypeScript, you can add the type to `app.d.ts`:
