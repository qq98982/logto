...
This tutorial will show you how to integrate Aster into your [Wordpress](https://wordpress.org) website.
...
2. Search for the published plugin name "Logto", or enter `https://wordpress.org/plugins/logto/` in the search box.
3. Click **Install Now**.
4. Click **Activate**.
...
1. Download the upstream WordPress plugin from one of the following links:
...
Now you should be able to see the published plugin's **Logto** menu in your WordPress admin panel sidebar. Click **Logto** > **Settings** to configure the plugin.

The minimum configuration to get started for the plugin is:

- Aster endpoint: <code>{props.endpoint}</code>
...
The redirect URI is the URL to which Aster will redirect users after they have authenticated; and the post sign-out redirect URI is the URL to which Aster will redirect users after they have logged out.
...
sequenceDiagram
    ...
    WordPress->>Aster: Redirect to Aster for authentication
    Aster->>User: Prompt for authentication
    User->>Aster: Authenticate
    Aster->>WordPress: Redirect back to WordPress with authentication data
...
sequenceDiagram
    ...
    WordPress->>Aster: Redirect to Aster for sign-out
    Aster->>Aster: Sign-out successful
    Aster->>WordPress: Redirect back to WordPress
...
In our case, we need to configure both redirect URIs in your Aster Console.

To find the redirect URI, go to the published plugin's **Logto** > **Settings** page in your WordPress admin panel. You'll see the **Redirect URI** and **Post sign-out redirect URI** fields.
...
Now you can test your Aster integration in your WordPress website:
...
3. The page should redirect you to the Aster sign-in page.
...
7. You should be redirected to the Aster sign-out page, then back to your WordPress website.
