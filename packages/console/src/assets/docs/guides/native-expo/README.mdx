...
<Step
  title="Install SDK"
  subtitle="Install the upstream SDK and peer dependencies"
>
...
The `@logto/rn` package is the SDK for Aster. The remaining packages are its peer dependencies. They couldn't be listed as direct dependencies because the Expo CLI requires that all dependencies for native modules be installed directly within the root project's `package.json`.
...
<Step title="Init Aster provider">

Import and use `LogtoProvider` to provide an Aster context:
