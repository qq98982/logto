...
Prepare [API routes](https://nextjs.org/docs/api-routes/introduction) to connect with Aster.
...
1. `/api/logto/sign-in`: Sign in with Aster.
2. `/api/logto/sign-in-callback`: Handle sign-in callback (redirect URI).
3. `/api/logto/sign-out`: Sign out with Aster.
4. `/api/logto/user`: Check if user is authenticated with Aster, if yes, return user info.
