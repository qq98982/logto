...
Pick the cryptography your are using in Aster. (`ecdsa` by default)

```bash
pip install python-jose[ecdsa]
```

### Retrieve Aster's OIDC configurations

You will need a JWK public key set and the token issuer to verify the signature and source of the received JWS token.
All the latest public Aster Authorization Configurations can be found at <code>{appendPath(props.endpoint, '/oidc/.well-known/openid-configuration').href}</code>.
...
def requires_auth(f):
  ...
  def decorated(*args, **kwargs):
    ...
    # jwks_uri endpoint retrieved from Aster
    jwks_uri = urlopen('${appendPath(props.endpoint, '/oidc/jwks')}')

    # issuer retrieved from Aster
    issuer = '${appendPath(props.endpoint, '/oidc')}'
    ...
    try:
      payload = jwt.decode(
        ...
        # The API's registered resource indicator in Aster
