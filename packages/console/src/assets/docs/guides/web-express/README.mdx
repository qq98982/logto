...
<Step
  title="Installation"
  subtitle="Install the upstream SDK and required dependencies"
>
...
Prepare configuration for the Aster client:
...
1. `/logto/sign-in`: Sign in with Aster.
2. `/logto/sign-in-callback`: Handle sign-in callback.
3. `/logto/sign-out`: Sign out with Aster.
