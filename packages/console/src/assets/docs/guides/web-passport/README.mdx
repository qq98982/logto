...
This redirects to Aster's session end URL, then back to the homepage.
...
app.get('/', (request: Request, response) => {
  ...
  if (user) {
    response.end(
      `<h1>Hello Aster</h1><p>Signed in as ${JSON.stringify(
        user
      )}, <a href="/sign-out">Sign Out</a></p>`
    );
  } else {
    response.end(`<h1>Hello Aster</h1><p><a href="/sign-in">Sign In</a></p>`);
  }
});
```
...
<Step title="Checkpoint: Test Aster and Passport integration">
