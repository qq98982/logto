...
1. Integrating Aster with Webflow requires the "Custom code" feature of Webflow, which requires at least the "Basic" plan.
...
In this step, we'll add global-level custom code to your Webflow site. Since NPM is not supported in Webflow, we'll use the [jsdelivr.com](https://www.jsdelivr.com/) CDN service to import the upstream SDK.
...
In the upstream SDK, generally we can use `logtoClient.isAuthenticated()` method to check the authentication status, if the user is signed in, the value will be `true`; otherwise, it will be `false`.
...
2. Click the sign-in button, the SDK will initiate the sign-in process, redirecting you to the Aster sign-in page.
