...
1. **CallbackPath**: The URI that Aster will redirect the user back to after the user has signed in (the "redirect URI" in Aster)
2. **RedirectUri**: The URI that will be redirected to after necessary actions have been taken in the Aster authentication middleware.
...
graph LR
  ...
  A(Sign-in path) -->|Redirect to| B(Aster)
...
| -------------------------------- | --------------------- |
| Aster redirect URI               | CallbackPath          |
| Aster post sign-out redirect URI | SignedOutCallbackPath |
