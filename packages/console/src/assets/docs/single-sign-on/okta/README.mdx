...
2. Add a new `Sign-in redirect URIs` using the Aster SSO connector's callback URL.
...
<Step index={2} title="Set up Aster connector with the client credentials">
...
**Tip:** If your app requests these scopes to access Okta APIs and perform actions, make sure to enable **Store tokens for persistent API access** in Aster Okta connector. See the next section for details.

</Step>

<Step index={5} title="Store tokens to access Okta APIs (Optional)">

If you want to access [Okta scopes](https://developer.okta.com/docs/guides/request-user-consent/main/#enable-consent-for-scopes) and perform actions with user authorization, Aster needs to get specific scopes and store tokens.

1. Add the required scopes in your Okta developer console API permissions configuration and Aster Okta connector.
2. Enable **Store tokens for persistent API access** in Aster Okta connector. Aster will securely store Okta access and refresh tokens in the Secret Vault.
3. To ensure refresh tokens are returned, add the `offline_access` scope to your Okta application permissions and include it in your Aster Okta connector scopes. This scope allows your application to maintain access to resources for extended periods.
