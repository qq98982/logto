const connectors = {
  ...
  guide: {
    ...
    target_tooltip:
      'Aster 社交連接器的「target」指的是社交身份的「來源」。在 Aster 的設計裡，我們不允許某一平台的連接器中有相同的「target」以避免身份的衝突。在添加連接器時，你需要格外小心，我們「不允許」用戶在創建之後更改「target」的值。<a>了解更多</a>',
    target_conflict:
    ...
    callback_uri_custom_domain_description:
      '如果你在 Aster 使用多個<a>自訂網域</a>，務必把所有對應的回呼 URI 加到 IdP，確保社群登入在每個網域都能運作。\n\n預設的 Aster 網域 一直有效；只有在你也希望支援該網域下的登入時才需要包含它。',
    acs_url: '斷言消費者服務網址',
  },
  platform: {
    universal: '通用',
