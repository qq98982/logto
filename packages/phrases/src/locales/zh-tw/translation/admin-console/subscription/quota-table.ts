const quota_table = {
  ...
  mau_tip: 'MAU（每月活躍用戶）是指在計費週期內與 Aster 交換過至少一個令牌的獨立用戶數量。',
  tokens_tip: 'Aster 發行的所有類型令牌，包括訪問令牌、刷新令牌等。',
  mao_tip: 'MAO（月度活躍組織）指的是在計費週期內至少有一個 MAU（月度活躍用戶）的獨特組織數量。',
  third_party_tip: '使用 Aster 作為你的 OIDC 身份提供程序，以便第三方應用程式進行登錄和權限授予。',
