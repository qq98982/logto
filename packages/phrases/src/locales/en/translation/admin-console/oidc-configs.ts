const oidc_configs = {
  sessions_card_title: 'Aster sessions',
  sessions_card_description:
    "Customize the session policy stored by the Aster authorization server. It records the user's global auth state to enable SSO and allow silent re-authentication across apps.",
  session_max_ttl_in_days: 'Session maximum time to live (TTL) in days',
  session_max_ttl_in_days_tip:
    'An absolute lifetime limit from session creation. Regardless of activity, the session ends when this fixed duration elapses.',
  cloud_private_key_rotation_notice:
    'In Cloud, private key rotation takes effect after a 4-hour grace period.',
};

export default Object.freeze(oidc_configs);
