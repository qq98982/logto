const enterprise_sso = {
  ...
  placeholder_description:
    'Aster предоставляет множество встроенных поставщиков идентичности предприятия для подключения, в то же время вы можете создать собственного поставщика с использованием протоколов SAML и OIDC.',
  create_modal: {
  ...
  basic_info: {
    ...
    saml: {
      ...
      sign_auth_request_tooltip:
        'Aster подписывает SAML-запросы аутентификации сгенерированным сертификатом. Включайте только если ваш поставщик удостоверений настроен на проверку подписанных запросов.',
    ...
    oidc: {
      ...
      redirect_uri_field_custom_domain_description:
        'Если вы используете в Aster несколько <a>пользовательских доменов</a>, обязательно добавьте все соответствующие URI обратного вызова в IdP, чтобы SSO работал на каждом домене.\n\nДомен Aster по умолчанию всегда действителен — включайте его только если хотите поддерживать SSO и на этом домене.',
    },
  },
  attribute_mapping: {
    ...
    col_sp_claims: 'Значение поставщика услуг (Aster)',
