...
const subscription = {
  ...
  free_plan_description: 'Для побочных проектов и начальных испытаний Aster. Без кредитной карты.',
  pro_plan: 'Про план',
  pro_plan_description: 'Позволяет бизнесу использовать Aster без забот.',
  ...
  payment_error:
    'Обнаружена ошибка платежа. Невозможно обработать сумму ${{price, number}} за предыдущий цикл. Обновите платежную информацию, чтобы избежать блокировки сервиса Aster.',
  ...
  not_eligible_modal: {
    ...
    upgrade_description:
      'В данный момент вы используете больше, чем позволяет <name />. Aster теперь официально, включая функции, специально разработанные для каждого тарифного плана. Прежде чем рассматривать обновление до <name />, убедитесь, что вы соответствуете следующим критериям перед обновлением.',
  ...
  token_usage_notification: {
    ...
    close_to_limit:
      'Вы почти достигли предела использования токенов. Aster перестанет предоставлять токены, если ваше использование превысит 100%. Пожалуйста, обновите Бесплатный план, чтобы избежать неудобств.',
    dev_plan_exceeded:
      'Этот арендатор достиг предела токенов согласно политике ограничения сущности Aster.',
  },
};
