const domain = {
  ...
  update_endpoint_notice:
    'Не забудьте обновить домен для обратного вызова социального коннектора и конечной точки Aster в вашем приложении, если вы хотите использовать пользовательский домен для функций.',
  error_hint:
  ...
  custom: {
    ...
    deletion: {
      ...
      in_used_tip:
        'Если ранее вы настраивали этот пользовательский домен в своём провайдере социальных коннекторов или конечной точке приложения, вам нужно сначала изменить URI на домен по умолчанию Aster "<span>{{domain}}</span>". Это необходимо для правильной работы кнопки социальной авторизации.',
  ...
  default: {
    ...
    default_domain_description:
      'Aster предлагает предварительно настроенный домен по умолчанию, готовый к использованию без дополнительной настройки. Этот домен по умолчанию служит как резервный вариант, даже если вы включили пользовательский домен.',
    default_domain_field: 'Домен по умолчанию Aster',
  },
