const connectors = {
  ...
  placeholder_description:
    'Aster предоставляет множество широко используемых коннекторов для входа через социальные сети, в то время как вы можете создать свой с помощью стандартных протоколов.',
  ...
  guide: {
    ...
    target_tooltip:
      '"Название поставщика идентификации" в социальных коннекторах Aster относится к "источнику" ваших социальных пользователей. В дизайне Aster мы не принимаем то же самое "название поставщика идентификации" для определенной платформы, чтобы избежать конфликтов. Вы должны быть очень внимательны, прежде чем добавлять коннектор, так как вы НЕ МОЖЕТЕ изменить его значение после создания. <a>Узнать больше</a>',
    target_conflict:
    ...
    callback_uri_custom_domain_description:
      'Если вы используете в Aster несколько <a>пользовательских доменов</a>, обязательно добавьте все соответствующие URI обратного вызова в IdP, чтобы социальный вход работал на каждом домене.\n\nДомен Aster по умолчанию всегда действителен - включайте его только если хотите поддерживать входы и на этом домене.',
