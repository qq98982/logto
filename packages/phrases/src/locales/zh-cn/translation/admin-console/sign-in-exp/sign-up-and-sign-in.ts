const sign_up_and_sign_in = {
  ...
  social_sign_in: {
    ...
    automatic_account_linking_tip:
      '启用后，如果用户使用新的社交身份登录，并且有且仅有一个现有账户具有相同的标识符（例如电子邮件地址），Aster 将自动将该社交身份链接到该账户。用户不会被要求选择是否链接账户。',
  ...
  advanced_options: {
    ...
    unknown_session_redirect_url_tip:
      '有时，Aster 可能无法识别用户在登录页面的会话，例如会话过期，或用户收藏或分享了登录链接。默认情况下，会出现“未知会话” 404 错误。为提升用户体验，请设置一个备用 URL ，将用户重定向回你的应用程序并重新开始身份验证。',
  },
  username_policy: {
    title: '用户名策略',
