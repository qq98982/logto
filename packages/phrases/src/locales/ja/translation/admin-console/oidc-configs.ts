const oidc_configs = {
  sessions_card_title: 'Aster セッション',
  sessions_card_description:
    'Aster 認可サーバーに保存されるセッションポリシーをカスタマイズします。ユーザーのグローバル認証状態を記録し、SSO を有効化してアプリ間でのサイレント再認証を可能にします。',
  ...
  cloud_private_key_rotation_notice:
    'Cloud では、プライベートキーのローテーションは 4 時間の猶予期間後に有効になります。',
};
