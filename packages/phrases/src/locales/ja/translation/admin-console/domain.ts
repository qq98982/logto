const domain = {
  ...
  update_endpoint_notice:
    'カスタムドメインを使用する場合は、アプリケーションのソーシャルコネクタコールバックURIとAsterエンドポイントのドメインを更新することを忘れないでください。',
  ...
  custom: {
    ...
    deletion: {
      ...
      in_used_tip:
        '以前にソーシャルコネクタプロバイダまたはアプリ終了ポイントでこのカスタムドメインを使用して設定した場合、それらのURIをAsterデフォルトドメイン「<span>{{domain}}</span>」に変更する必要があります。これは、ソーシャルサインインボタンを機能させるために必要です。',
  ...
  default: {
    ...
    default_domain_description:
      'Asterは、追加のセットアップなしで使用できる事前に構成されたデフォルトドメインを提供しています。このデフォルトドメインは、カスタムドメインを有効にしていない場合でもバックアップオプションとして機能します。',
    default_domain_field: 'Asterデフォルトドメイン',
  },
