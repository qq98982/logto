const connectors = {
  ...
  placeholder_description:
    'Aster は、標準プロトコルを使用して独自のソーシャルサインインコネクタを作成できるとともに、多くの一般的に使用されているソーシャルサインインコネクタを提供しています。',
  ...
  guide: {
    ...
    target_tooltip:
      'Aster のソーシャルコネクタでの「IdP 名」とは、ソーシャル ID の「ソース」を指します。「IdP 名」は、Platform ごとに一意でなければならず、同じ名前は許可されません。「IdP 名」は一度作成されたコネクタで変更することはできません。<a>詳細を見る</a>',
    target_conflict:
    ...
    callback_uri_custom_domain_description:
      'Aster で複数の<a>カスタムドメイン</a>を使用する場合は、すべての対応するコールバック URI を IdP に追加し、各ドメインでソーシャルログインが機能するようにしてください。\n\nAster の既定ドメインは常に有効です。そのドメインでのログインもサポートしたい場合にのみ含めてください。',
