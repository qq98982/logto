const quota_table = {
  ...
  mau_tip:
    'MAU（月間アクティブユーザー）は、請求サイクルで Aster と少なくとも 1 つのトークンを交換したユニークユーザーの数を指します。',
  tokens_tip:
    'Aster によって発行されたすべての種類のトークン、アクセストークン、リフレッシュトークンなどを含みます。',
  mao_tip:
    'MAO（月間アクティブ組織）は、請求サイクル内で少なくとも 1 つの MAU（月間アクティブユーザー）を持つユニークな組織の数を意味します。',
  third_party_tip:
    'Aster を OIDC アイデンティティプロバイダーとして使用して、サードパーティーアプリケーションのサインインや権限の付与を行います。',
