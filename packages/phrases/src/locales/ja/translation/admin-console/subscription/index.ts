...
const subscription = {
  ...
  free_plan_description:
    'サイドプロジェクトや最初の Aster のトライアルに適しています。クレジットカードは必要ありません。',
  pro_plan: 'プロプラン',
  pro_plan_description: 'ビジネスが安心して Aster を利用できるプランです。',
  ...
  payment_error:
    '支払いに問題が発生しました。前回のサイクルで ${{price, number}} を処理できませんでした。Aster のサービス停止を回避するために支払いを更新してください。',
  downgrade: 'ダウングレード',
  current: '現在',
  upgrade: 'アップグレード',
  quota_table,
  ...
  not_eligible_modal: {
    ...
    upgrade_description:
      '現在、<name /> の許容範囲を超えて使用しています。Aster は正式にリリースされ、各プランに合わせた機能が提供されています。 <name /> へのアップグレードを検討する前に、アップグレードする前提条件を満たしていることを確認してください。',
  ...
  token_usage_notification: {
    ...
    close_to_limit:
      'トークン使用制限にほぼ達しました。使用量が 100％を超えると Aster はトークンの発行を停止します。問題を避けるためにフリープランをアップグレードしてください。',
    dev_plan_exceeded:
      'このテナントは Aster のエンティティ制限ポリシーごとのトークン制限に達しました。',
  },
};
